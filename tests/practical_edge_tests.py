"""
Praktyczne testy EdgeHarmonize na rzeczywistych przypadkach użycia.
Testuje 3 scenariusze: portret, samochód, krajobraz.
"""
import json
import requests
import time
...
# Test cases from existing experiments
TEST_CASES = [
    {
        "name": "portrait_leather_jacket",
        "source": r"C:\Users\Mierz\Desktop\RasterRelay\tests\manual\test-images\envato-labs-ai-da532839-090d-4b70-9e60-1ed61c2e94a5.jpg",
    ...
    {
        "name": "car_recolor",
        "source": r"C:\Users\Mierz\Desktop\RasterRelay\tests\manual\test-images\car-color-test.jpg",
        "mask": r"C:\Users\Mierz\Desktop\RasterRelay\tests\manual\test-images\rr-04-car-logo-clean-panel-0131279d-mask.png",
        "prompt": "Edit the masked area only. Keep the result photorealistic and naturally integrated with the original image. Match perspective, lighting, shadows, exposure, color temperature, contrast, grain, texture sharpness and depth of field. Preserve unmasked areas exactly. Avoid visible seams, halos, pasted-on edges and color shifts.",
        "negative_prompt": "blurry, distortion, visible seam, halo, pasted edge, color mismatch, exposure mismatch, different lighting, watermark, text, low quality, jpeg artifacts, oversaturated, undersaturated",
        "seed": 42,
    ...
    {
        "name": "landscape_add_car",
        "source": r"C:\Users\Mierz\Desktop\RasterRelay\tests\manual\test-images\61CiqMYiRPL._AC_UY1000_.jpg",
        "mask": r"C:\Users\Mierz\Desktop\RasterRelay\tests\manual\test-images\rr-04-car-logo-clean-panel-0131279d-mask.png",
        "prompt": "Edit the masked area only. Keep the result photorealistic and naturally integrated with the original image. Match perspective, lighting, shadows, exposure, color temperature, contrast, grain, texture sharpness and depth of field. Preserve unmasked areas exactly. Avoid visible seams, halos, pasted-on edges and color shifts.",
        "negative_prompt": "blurry, distortion, visible seam, halo, pasted edge, color mismatch, exposure mismatch, different lighting, watermark, text, low quality, jpeg artifacts, oversaturated, undersaturated",
        "seed": 42,
...
OUTPUT_DIR = r"C:\Users\Mierz\Desktop\RasterRelay\tests\manual\test-results"
...
def create_workflow(source_image, mask_image, prompt, negative_prompt, seed, include_edge_harmonize=True, prefix="test"):
    ...
    if include_edge_harmonize:
        ...
        workflow["80"] = {"class_type": "RasterRelaySaveImage", "inputs": {"filename_prefix": f"{prefix}_with_edge", "images": ["98", 0]}}
    else:
        workflow["80"] = {"class_type": "RasterRelaySaveImage", "inputs": {"filename_prefix": f"{prefix}_without_edge", "images": ["95", 0]}}
...
def compute_edge_metrics(original_path, result_path, mask_path):
    ...
    if mask.shape[:2] != result.shape[:2]:
        ...
        mask_img = mask_img.resize((result.shape[1], result.shape[0]), Image.NEAREST)
    ...
    if edge_zone.sum() == 0:
        return {"edge_luma_diff": 0, "edge_color_diff": 0, "global_diff": 0, "edge_pixels": 0}
    ...
    return {
        ...
        "edge_pixels": int(edge_zone.sum()),
    }

def main():
    print("=" * 70)
    print("PRAKTYCZNE TESTY EDGEHARMONIZE NA RZECZYWISTYCH PRZYPADKACH")
    print("=" * 70)
    ...
    for i, test_case in enumerate(TEST_CASES):
        ...
        history = wait_for_completion(prompt_id)
        filename = get_output_image(history)
        with_path = os.path.join(OUTPUT_DIR, f"{test_case['name']}_with_edge.png")
    ...
    # Summary
    print("\n" + "=" * 70)
    print("PODSUMOWANIE TESTÓW PRAKTYCZNYCH")
    print("=" * 70)
    ...
    print("\nSzczegółowe wyniki:")
    print(f"{'Test':<25} {'Luma bez':>10} {'Luma z':>10} {'Poprawa':>10} {'Kolor bez':>10} {'Kolor z':>10} {'Poprawa':>10}")
    print("-" * 85)
    for r in results:
        print(f"{r['name']:<25} {r['without']['edge_luma_diff']:>10.2f} {r['with']['edge_luma_diff']:>10.2f} {r['luma_improvement']:>9.1f}% {r['without']['edge_color_diff']:>10.2f} {r['with']['edge_color_diff']:>10.2f} {r['color_improvement']:>9.1f}%")

    print("\n" + "=" * 70)
    if avg_luma_improvement > 50:
        print("WNIOSK: EdgeHarmonize ZNACZNIE POPRAWIA jakość krawędzi we wszystkich testach")
    ...
    print("=" * 70)
    print("Aby zobaczyć różnicę, porównaj pary obrazów:")
    for r in results:
        print(f"  - {r['name']}_without_edge.png vs {r['name']}_with_edge.png")
    print("\nZwróć uwagę na krawędzie zaznaczonego obszaru - czy halo/obwódka jest widoczna?")
