...
│   ├── area_match.py           # Dopasowanie obszaru (biblioteka)
│   ├── mask_cropper.py         # Przycinanie maski
│   ├── background_preserve.py  # Zachowanie tła (biblioteka)
│   ├── seamless_tone.py        # AKTYWNY: bezszwowy ton (dyfuzja LF)
...
3. ComfyUI Nodes przetwarzają (kolejność z inpainting-api.json):
   a) (VAEEncode + SetLatentNoiseMask + sampler Flux) - generacja w masce
   b) match_and_align.py  - VaeDriftMatch twardo przywraca piksele POZA maską
   c) seamless_tone.py    - SeamlessTone dopasowuje jasność/kolor wnętrza maski
                            do otoczenia (dyfuzja tonu niskiej częstotliwości)
   d) pad_to_document.py  - rozszerza wynik do rozmiaru dokumentu z alfą
   ↓
...
> **Dlaczego SeamlessTone.** Globalny transfer Reinharda (jedna średnia/odchylenie
> na cały obszar — stare `AreaMatch` + `ColorHarmonize`) nie radzi sobie z
> przestrzennie zmiennym oświetleniem i w pomiarach **pogarszał** szew
> (ΔL_szwu 0.0207 → 0.0223). `SeamlessTone` dyfunduje ton otoczenia do wnętrza
> maski i przesuwa tylko niską częstotliwość generacji → jasność wnętrza trafia
> w otoczenie (0.293 → 0.260, cel 0.262), a szew spada o ~26%. Zachowuje detal
> generacji i naprawia także gradienty światła.
>
